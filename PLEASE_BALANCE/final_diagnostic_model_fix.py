...
patientdata:pd.DataFrame = pd.read_csv("/home/dunnnm2/thesis_research/PLEASE_BALANCE/patient_data_modified.csv")
...
def extract_init_column_data(patient_data:pd.DataFrame, id_column:str='icustayid', binary_columns:List[str]=None, normal_columns:List[str]=None,
    ...
    colnorm:List[str] = [] 
    collog:List[str] = []
    colbin_df:pd.DataFrame = None # patient_data[colbin]
    colnorm_df:pd.DataFrame = None # patient_data[colnorm]
    collog_df:pd.DataFrame = None # patient_data[collog]
    ...
    if binary_columns == None:
        ...
        if colbin != []:
            colbin_df = patient_data[colbin]
        else:
            colbin_df = pd.DataFrame()
    else:
        colbin = binary_columns
        colbin_df = patient_data[colbin]
    
    if normal_columns == None:
        # colnorm = ['age', 'Chloride']
        ...
        if colnorm != []:
            colnorm_df = patient_data[colnorm]
        else:
            colnorm_df = pd.DataFrame()
    else:
    ...
    if log_columns == None:
        ...
        if collog != []:
            collog_df = patient_data[collog]
        else:
            collog_df = pd.DataFrame()
    else:
        collog = log_columns
        collog_df = patient_data[collog]
...
def balance(train_set_predict:List[int], train_set_response:List[int], is_debug:bool) -> List[int]:
    ...
    train_set_predict_bal = pd.DataFrame(train_set_predict_bal, columns=start_column_headers)
...
def crossval_split(stratified_data:pd.DataFrame, predictor_columns:List[str], 
    ...
    all_train_sets_inner = []
    all_test_sets_inner = []
    all_train_sets_outer = []
    all_test_sets_outer = []
    ...
    for training_samples, test_samples in outer_cv.split(predict_data, response_data):
        subset_train_set_inner = []
        subset_test_set_inner = []
        for inner_train, inner_test in inner_cv.split(predict_data.iloc[training_samples], response_data.iloc[training_samples]):
            ...
            subset_train_set_inner.append(train_ids)
            subset_test_set_inner.append(test_ids)
            all_train_sets_inner.append(subset_train_set_inner)
            all_test_sets_inner.append(subset_test_set_inner)
        
        # Grab the training data indexes and corresponding reponses
        train_predict = predict_data.iloc[training_samples]
        train_response = response_data.iloc[training_samples]
        # Grab the testing data indexes and corresponding responses
        test_predict = predict_data.iloc[test_samples]
        train_ids = []
        if balance_flag:
            # Balance the training dataset, grab the desired ids
            train_ids = balance(train_predict, train_response, is_debug=False)
        ...
        test_ids = list(map(id_correction_dict.get, test_ids))
        # Load them into a grand list
        all_train_sets_outer.append(train_ids)
        all_test_sets_outer.append(test_ids)
    return all_train_sets_inner, all_test_sets_inner, all_train_sets_outer, all_test_sets_outer
...
Output:
train_flag_set_inner - Flags of the training set to be tested on a corresponding testing set, Stratified
test_flag_set_inner - Flags of the testing set to evaluate a corresponding training set's model 
train_flag_set_outer - Flags of training set to optimize hyperparameters based on the training set
test_flag_set_outer - Flags of testing set to evaluate a training set's hyperparameters
"""
...
def cross_validate_and_balance(icu_ids:pd.DataFrame, debug_flag:bool, save_to_file:bool, 
    ...
    if regenerate_flag:
        ...
        all_train_sets_inner, all_test_sets_inner, all_train_sets_outer, all_test_sets_outer = crossval_split(icu_pair_set, predictor_columns, response_column, 
                                                     is_debug=False, id_correction_dict=id_correction_dict, balance_flag=True)
        train_flag_set_inner = []
        test_flag_set_inner = []
        train_flag_set_outer = []
        test_flag_set_outer = []
        
        for train_ids_set in all_train_sets_inner:
                subset_train_ids = []
                for train_ids in train_ids_set:
                        subset_train_ids.append(patientdata['icustayid'].isin(train_ids))
                train_flag_set_inner.append(subset_train_ids)
        for test_ids_set in all_test_sets_inner:
                subset_test_ids = []
                for test_ids in test_ids_set:
                        subset_test_ids.append(patientdata['icustayid'].isin(test_ids))
                test_flag_set_inner.append(subset_test_ids)
        for train_ids in all_train_sets_outer:
            train_flag_set_outer.append(patientdata['icustayid'].isin(train_ids))
        for test_ids in all_test_sets_outer:
            test_flag_set_outer.append(patientdata['icustayid'].isin(test_ids))
        ...
        if save_to_file:
            with open('train_flag_set_inner.txt', 'wb') as fp:
                pickle.dump(train_flag_set_inner, fp)
            with open('test_flag_set_inner.txt', 'wb') as fp:
                pickle.dump(test_flag_set_inner, fp)
            with open('train_flag_set_outer.txt', 'wb') as fp:
                pickle.dump(train_flag_set_outer, fp)
            with open('test_flag_set_outer.txt', 'wb') as fp:
                pickle.dump(test_flag_set_outer, fp)
            
        return train_flag_set_inner, test_flag_set_inner, train_flag_set_outer, test_flag_set_outer
    else:
        ...
        train_flag_set_outer = []
        test_flag_set_outer = []
        with open('train_flag_set_inner.txt', 'rb') as fp:
            train_flag_set_inner = pickle.load(fp)
        with open('test_flag_set_inner.txt', 'rb') as fp:
            test_flag_set_inner = pickle.load(fp)
        with open('train_flag_set_outer.txt', 'rb') as fp:
            train_flag_set_outer = pickle.load(fp)
        with open('test_flag_set_outer.txt', 'rb') as fp:
            test_flag_set_outer = pickle.load(fp)
        return train_flag_set_inner, test_flag_set_inner, train_flag_set_outer, test_flag_set_outer
...
def kmeans_cluster_calculations(train_zscores:pd.DataFrame, test_zscores:pd.DataFrame,
    ...
    if regenerate_flag:
        clusters_models = KMeans(n_clusters=state_count, max_iter=num_loops_per_iter,
                                 n_init=max_num_iter, random_state=12345678).fit(train_zscores)
        # Save the important data (clusters created as a result of the K-Means operations)
        # This process takes quite a while. This will provide a checkpoint to decrease compute time
        # until the code is put into dev.
...
#
# Ex: We apply the function to the action_column "IV_Fluid", which has split the data into 4 different groups of 
# IV_Fluid actions. This function will produce a list containing the min and max amount of IV_Fluid administered for each of those
# groups (Group 1 -> Adminster 20 mL, Group 2 -> Administer 40 mL, Group 3 -> Administer 60 mL, Group 4 -> Administer 80 mL
"""

def min_max_action_values(actions_column: pd.DataFrame, real_values:pd.DataFrame) -> List[np.float64]:
    # Grab all the unique actions for a column and sort them
    all_groups:List[np.float64] = np.sort(actions_column.unique())
    # Concatanate the group number and real value for each row
    action_set:pd.DataFrame = pd.concat([actions_column, real_values], axis=1, sort=False)
    ...
    action_set.columns = ['group_id', 'data_val']
    # Grab the median value for each group based on group number using python list comprehension
    median_values:List[np.float64] = [[np.min(action_set[action_set['group_id'] == i]['data_val']), 
                                       np.max(action_set[action_set['group_id'] == i]['data_val'])] for i in all_groups]
...
def build_dataset_actions(patientdata:pd.DataFrame, first_column:str="SOFA", second_column:str="qSOFA", num_groups_first_column:int=5,
    ...
    # Obtain the real min and max values that each division represents
    ...
    first_minmax_actions:List[np.float64] = min_max_action_values(actions_column = first_col_actions, real_values = patientdata[first_column])
    second_minmax_actions:List[np.float64] = min_max_action_values(actions_column = second_col_actions, real_values = patientdata[second_column])
    
    if debug_flag:
        print(first_column," Action Median Values:", str(first_minmax_actions), "\n" + second_column + ":", second_minmax_actions, "\n")
    ###
    ...
    if debug_flag:
        print(first_minmax_actions, second_minmax_actions)
        for i in range(0, len(action_keys)):
            first_col_index = action_keys[i][0] - 1
            second_col_index = action_keys[i][1] - 1
            first_col_min = first_minmax_actions[first_col_index][0]
            first_col_max = first_minmax_actions[first_col_index][1]
            second_col_min = second_minmax_actions[second_col_index][0]
            second_col_max = second_minmax_actions[second_col_index][1]
            full_format_string = (
                f'Action {i} accounts for all {first_column} values'                 
                f' in the inclusive range: {first_col_min}-{first_col_max}'
                f' and all {second_column} values'
                f' in the inclusive range: {second_col_min}-{second_col_max}'            
            )
    ...
    return train_chosen_actions, test_chosen_actions, action_list, action_count
...
def eval_predict_correct(reward_value:int, correct_answer:int):
    ...
    NEGATIVE_REWARD = -100
    POSITIVE_REWARD = 100
    # In the case that the reward value was nothing, simply return the correct answer
    if reward_value == 0:
        if correct_answer == PATIENT_DIED:
...
"""
# modify_incoming_data takes the existing data prepared and ready to go into the Q-Learning algorithm
# and modifies the order of the rows for testing
#
# Input: 
# ql_train_set - Default, normal Q-Learning dataset ready to go
# data_option - One of the following:
#   normal: Same as the input
#   striped: Living and Dead alternated for as long as the data lasts
#   all_dead_first: All the dead patients inputted to the set first
#   all_living_first: All the living patients inputted to the set first
#
"""
def modify_incoming_data(ql_train_set_Q:pd.DataFrame, data_option:str):
    ...
    if data_option == "normal":
        print(len(patient_indexes['patient_id'].tolist()))
        pass
        ...
        all_dead_list:List[str] = all_dead['patient_id'].tolist()
...
"""
def offpolicy_Q_learning_eval(ql_train_set_Q: pd.DataFrame, gamma: float, 
                              numtraces: int, num_actions: int, 
                              num_clusters: int, stopping_difference: float, 
                              is_random=False, data_option="all_dead_first",
    ...
    # Strangely, Python has no support for a 3D array of empty lists
    Q_Equation:int_matrix3D = np.zeros((num_actions, num_clusters, numtraces))
    Q_Equation[:] = np.nan
    # We need to keep track of the counts as well
    Q_Counts:int_matrix2D = np.zeros((num_actions, num_clusters), dtype=int)
    ...
    for i in range(0, numtraces):
        ...
        # bonus_multiplier_reduction = 0.1
        for j in range(trace_length - 2, - 1, -1):
            ...
            q_curr_index:int = Q_Counts[step_action, step_state]
            # Add the reward to the overall list for the state-action pairing, to be averaged at the end
            Q_Equation[step_action, step_state, q_curr_index] = return_reward 
            ...
            # current iteration
            Q_Counts[step_action, step_state] = Q_Counts[step_action, step_state] + 1
            # bonus_multiplier = bonus_multiplier * bonus_multiplier_reduction
    ...
    # Calculate averages for all the interim Q-Equation values to finalize the model
    Q_Equation_final:int_matrix2D = np.zeros((num_actions, num_clusters))
    for i in range(0, num_actions):
        for j in range(0, num_clusters):
            # Grab the number of times the state-action pairing was hit
            curr_count = Q_Counts[i, j]
            if curr_count == 0:
                Q_Equation_final[i, j] = 0
            else:
                Q_Equation_final[i, j] = np.nanmean(Q_Equation[i, j, :curr_count])
                # Cap the range for node values (-100, 100)
                if Q_Equation_final[i, j] > 100:
                    Q_Equation_final[i, j] = 100
                if Q_Equation_final[i, j] < -100:
                    Q_Equation_final[i, j] = -100
    # Trim off the portion of the list we did not use
    sum_Q_values = sum_Q_values[0:jj]
    return Q_Equation_final, sum_Q_values
...
# do_print - Prints the values to a file
#
# Output:
# total_string - Full Body of information provided by evaluation step
# overall_accuracy - Decimal Representation of the accuracy of the model
"""
def evaluate_model_accuracy(qlearning_train_final:pd.DataFrame, test_name:str, is_debug:bool, gamma:float, run_num:int, do_print:bool):
    ...
    # Just for checking false negatives (Dead people predicted to live)
    average_count_negative:List[int] = []
    average_count_positive:List[int] = []
    for id_val in all_patients_ids:
        patient_values = qlearning_train_final[qlearning_train_final['patient_id'] == id_val]
        all_rewards:pd.Series = patient_values['reward_value']
        all_actions:pd.Series = patient_values['chosen_action_index']
        # We are not supposed to know the terminal reward for evaluation, this is the life/death state
        last_action:pd.Series = all_actions.iloc[-2]
        ...
        reward_sign:int = np.sign(reward_sum)
        # Treat 0s as Living
        if reward_sign == 0:
        ...
        # If the patient was presumed to be alive and died, count it
            ...
            false_negative_ids.append(id_val)
    ...
    total_string = (f'{run_num},{gamma},{total_patients},{correct_guesses},'
    ...
    return total_string, overall_accuracy
...
def aggregate_dataset_construct(debug_flag:bool = False, save_to_file:bool = True, 
                                regenerate_flag:bool=True, regenerate_cross:bool=True):
    ...
    train_flag_set_inner, test_flag_set_inner, train_flag_set_outer, test_flag_set_outer = cross_validate_and_balance(icu_ids, debug_flag, 
                                                                             save_to_file, regenerate_flag, 
                                                                             patientdata, regenerate_cross,
                                                                             all_factors)
    # The flags are all we need for the subsequent steps
    return train_flag_set_inner, test_flag_set_inner, train_flag_set_outer, test_flag_set_outer, MIMIC_zscores
...
def aggregate_training_clustering(train_flag:pd.DataFrame, test_flag:pd.DataFrame, MIMIC_zscores:pd.DataFrame,
                                  is_debug:bool=False, save_flag:bool=True, variance_allowed=0.95, 
    ...
    # Training and Testing Construction based on cluster outputs #is_debug
    train_chosen_actions, test_chosen_actions, action_list, action_count = build_dataset_actions(patientdata=patientdata, 
...
# qlearning_dataset_test_final_weighted - Q-Learning final model output for the weighted test set 
# qlearning_dataset_test_final_no_weighting - Q-Learning final model output for the unweighted test set
# gamma - Hyperparameter for use in print
# run_num - Number of run for evaluation purposes
# do_print - Whether or not to print results to file
#
# Output:
# overall_accuracy - How well did the model perform
# 
"""
def aggregate_Q_learning_evaluation(qlearning_dataset_train_final_weighted:pd.DataFrame, qlearning_dataset_train_final_no_weighting:pd.DataFrame,
                                    ...
                                    gamma:float, run_num:int, do_print:bool):
    total_string, overall_accuracy = evaluate_model_accuracy(qlearning_dataset_test_final_weighted, test_name="Test_Weighted", 
                                                             is_debug=True, gamma=gamma, run_num=run_num, do_print=do_print)
    total_string, overall_accuracy = evaluate_model_accuracy(qlearning_dataset_test_final_no_weighting, test_name="Test_No_Weighting", 
                                                             is_debug=True, gamma=gamma, run_num=run_num, do_print=do_print)
...
# In[244]:
def final_model_run_parallel(run_val, train_flag_set_inner, test_flag_set_inner, train_flag_set_outer, test_flag_set_outer, MIMIC_zscores, all_gammas):
	total_needed_runs:List[int] = [i for i in range(0, len(train_flag_set_outer))]
	thread_needed_runs:List[int] = np.array_split(total_needed_runs, 10)[run_val]
	for i in thread_needed_runs:
		train_flag = train_flag_set_outer[i]
		test_flag = test_flag_set_outer[i]
		...
		hyper_opt_results = [[0.0 for i in range(0, len(train_flag_set_inner))] for i in range(0, len(all_gammas))]
		# Conduct Hyper-Parameter Optimization using grid results
		for k in range(0, len(train_flag_set_inner[i])):
			train_flag_inner = train_flag_set_inner[i][k]
			test_flag_inner = test_flag_set_inner[i][k]
			qlearning_dataset_train_final_inner, qlearning_dataset_test_final_inner, transition_mat_inner,             physician_policy_inner, total_actions, state_count =                     aggregate_training_clustering(train_flag_inner, test_flag_inner, MIMIC_zscores)
			for j in range(0, len(all_gammas)):
				gamma = all_gammas[j]
				# Perform Q-Learning to build finalized dataset with rewards
				qlearning_dataset_train_final_weighted_inner, qlearning_dataset_train_final_no_weighting_inner,                 qlearning_dataset_test_final_weighted_inner, qlearning_dataset_test_final_no_weighting_inner =                 aggregate_Q_learning_model_construction(qlearning_dataset_train_final_inner, 
														qlearning_dataset_test_final_inner,
														gamma=gamma, state_count=state_count, 
														total_actions=total_actions, physician_policy=physician_policy_inner)
				# Evaluate the functions
				overall_accuracy = aggregate_Q_learning_evaluation(qlearning_dataset_train_final_weighted_inner, qlearning_dataset_train_final_no_weighting_inner,
												qlearning_dataset_test_final_weighted_inner, qlearning_dataset_test_final_no_weighting_inner,
												gamma=gamma, run_num=i, do_print=False)
				# Add the accuracy for evaluation after all 10 runs
				hyper_opt_results[j][k] = overall_accuracy
		# Determine optimal from the set of all values
		averages_hyper_results = [np.sum(hyper_opt_results[i]) for i in range(0, len(hyper_opt_results))]
		# Grab the largest average and use that as gamma
		gamma = all_gammas[averages_hyper_results.index(max(averages_hyper_results))]
		# Perform Q-Learning to build finalized dataset with rewards
		...
		aggregate_Q_learning_evaluation(qlearning_dataset_train_final_weighted, qlearning_dataset_train_final_no_weighting,
												...
												gamma=gamma, run_num=i, do_print=True)



# This is the run loop to execute and save the results to the file for the Q-Learning Weighted and Unweighted Model
if __name__ == "__main__":
    train_flag_set_inner, test_flag_set_inner, train_flag_set_outer, test_flag_set_outer,     MIMIC_zscores = aggregate_dataset_construct()
    all_gammas = [0.01, 0.05, 0.10]
	# Run over 24 Cores
    jobs = []
    for run_val in range(0, 10):
        print(run_val)
        p = multiprocessing.Process(target=final_model_run_parallel, args=(run_val, train_flag_set_inner, test_flag_set_inner, train_flag_set_outer, test_flag_set_outer, MIMIC_zscores, all_gammas))
        jobs.append(p)
        p.start()
