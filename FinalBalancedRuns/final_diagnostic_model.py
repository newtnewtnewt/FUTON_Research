...
patientdata:pd.DataFrame = pd.read_csv("/home/dunnnm2/thesis_research/perhaps_i_may_become/patient_data_modified.csv")
...
# 
"""
def evaluate_model_accuracy(qlearning_train_final:pd.DataFrame, test_name:str, is_debug:bool, gamma:float, run_num:int):
    ...
    for id_val in all_patients_ids:
        ...
        for action in all_actions:
            ...
            if action >= 14:
                reward_sum = reward_sum + -20
        # If the second to last state is something very low in Q-Sofa and SOFA, positively weight it
        if last_action <= 4:
            reward_sum = reward_sum + 100
        # If the second to last state is something very high in the Q-SOFA and SOFA range, negatively weight it
        if last_action >= 14:
            reward_sum = reward_sum + -300 
        # If the reward is less than +100 and the sign is positive, the patient likely died 
...
def aggregate_Q_learning_evaluation(qlearning_dataset_train_final_weighted:pd.DataFrame, qlearning_dataset_train_final_no_weighting:pd.DataFrame,
                                    ...
                                    gamma:float, run_num:int):
    evaluate_model_accuracy(qlearning_dataset_test_final_weighted, test_name="Test_Weighted", is_debug=True, gamma=gamma, run_num=run_num)
    evaluate_model_accuracy(qlearning_dataset_test_final_no_weighting, test_name="Test_No_Weighting", is_debug=True, gamma=gamma, run_num=run_num)
...
# In[244]:
def final_model_run_parallel(run_val, train_flag_set, test_flag_set, MIMIC_zscores, all_gammas):
	total_needed_runs:List[int] = [i for i in range(0, len(train_flag_set))]
	thread_needed_runs:List[int] = np.array_split(total_needed_runs, 24)[run_val]
...
if __name__ == "__main__":
    ...
    all_gammas = [i/100.0 for i in range(1, 101)]
	# Run over 24 Cores
    jobs = []
    for run_val in range(0, 8):
        print(run_val)
        p = multiprocessing.Process(target=final_model_run_parallel, args=(run_val, train_flag_set, test_flag_set, MIMIC_zscores, all_gammas))
        jobs.append(p)
        p.start()
